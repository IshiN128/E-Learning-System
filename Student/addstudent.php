...
if(isset($_POST['stuemail']) && isset($_POST['checkemail'])){
 ...
 if(isset($_POST['stusignup']) && isset($_POST['stufname']) && isset($_POST['stulname']) && isset($_POST['stuemail']) && isset($_POST['stupass'])){
  ...
  $sql = "INSERT INTO student(stu_fname,stu_lname,stu_email,stu_pass,verification_code,is_active) VALUES ('$stufname','$stulname', '$stuemail', '$stupass', '$verification_code', false)";
 ...
 // Mail sending code
  ...
  if(!isset($_SESSION['is_login'])){
    if(isset($_POST['checkLogemail']) && isset($_POST['stuLogEmail']) && isset($_POST['stuLogPass'])){
      ...
      $sql = "SELECT stu_email, stu_pass FROM student WHERE stu_email='".$stuLogEmail."' AND stu_pass='".$stuLogPass."' AND is_active = true" ;
      $result = $conn->query($sql);
      $row = $result->num_rows;
